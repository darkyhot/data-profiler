...
- **Профиль значений** (`profiles/*.json`): типы, min/max (не для чувствительных),
  **все категории** (если уникальных ≤ `MAX_CATEGORIES`; `CATEGORIES_FULL_SCAN`
  добирает редкие скошенные значения — только по настоящим категориям: текст/enum/малые
  коды, метрики и даты в добор не идут; во временную таблицу `my_tab` материализуются
  ТОЛЬКО колонки-категории, затем `array_agg(DISTINCT)` ПАЧКАМИ по `CATEGORIES_SCAN_BATCH_COLS`
  колонок, с лимитом `CATEGORIES_SCAN_TIMEOUT_S` = 20 мин), кардинальность, доля null,
