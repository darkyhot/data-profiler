...
- **Профиль значений** (`profiles/*.json`): типы, min/max (не для чувствительных),
  **все категории** (если уникальных ≤ `MAX_CATEGORIES`; `CATEGORIES_FULL_SCAN`
  ...
  коды, метрики и даты в добор не идут; данные материализуются во временную таблицу
  `my_tab`, затем `array_agg(DISTINCT)`, с лимитом `CATEGORIES_SCAN_TIMEOUT_S` = 20 мин),
  кардинальность, доля null,
