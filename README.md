...
- **Профиль значений** (`profiles/*.json`): типы, min/max (не для чувствительных),
  **все категории** (если уникальных ≤ `MAX_CATEGORIES`; `CATEGORIES_FULL_SCAN`
  добирает редкие скошенные значения одним запросом на таблицу — только по настоящим
  категориям: текст/enum/малые коды, метрики и даты в добор не идут), кардинальность, доля null,
